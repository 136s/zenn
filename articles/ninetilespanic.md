...
3 つのお題すべてでそれぞれの最高点を取れるお題セットについては、「[お題セットごとの最適解](#お題セットごとの最適解)」にてコードと町のリストを記載しています。
...
全てのお題についての最高得点の距離の総和は、64-104 の範囲を取りました。
距離が 64 であるお題シノニムは 1 つで、そのリストは以下でした。

```text: Theme synonym ID: 903102
...
お題 10 と 15 は完全に反対のお題です。それぞれを除いた時についても確認してみます。
距離の総和は、お題 10 を除いた時は 61-103 で、お題 15 を除いた時は 62-102 の範囲を取りました。
それぞれ 61, 62 となったお題シノニムは上記のお題シノニムと変わりませんでした。
